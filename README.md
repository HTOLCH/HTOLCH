...
*Master's thesis, marked 87/100.* **[Code and write-up](https://github.com/HTOLCH/autoware_crowd_matching_module)**
...
pedestrian it sees, so on a busy path it never gets anywhere.
...
crowd never costs clearance.

To test it, I built a pedestrian simulation environment in AWSIM with the crowd driven by a Social
Force Model, which doubled as the benchmark baseline.

I also brought the full Autoware stack up on the shuttle's own hardware, integrating it from a large
and sparsely documented codebase, and built the lidar perception that tracks pedestrians around it.

Across a density sweep it removed stop events entirely and held mean time-to-collision above 2.3 s.
The planner was validated in simulation; the stack deployment ran on the vehicle.
...
simulation tend to surface.

Perception on PyTorch: object detectors to find targets around the course, and a vision-based policy
mapping camera images straight to steering.
